...
#data files
input_data_path = "/data/Amy/dynamicHR/HumanQA/analysis/final_analysis/"
...
for time_shift in time_shifts:
    ...
    for i, run in enumerate(runs):
        ...
        for train_run in train_runs:
            ...
            model_filename = task1 + "_var_mask02_trained_lasso_cj_" + time_snippet + ".pckl"
            model_path = os.path.join(output_data_path, session1, "models", "outputs", model_filename)
            ...
            #normalization_filename = task1 + "_retro_X_mean_std_" + time_snippet + "_v2.pckl"
            scale_filename = task1 + "_var_mask02_X_mean_" + time_snippet + ".pckl"
            #normalization_path = os.path.join(input_data_path, session1, "models", "outputs", normalization_filename)
            scale_path = os.path.join(output_data_path, session1, "models", "outputs", scale_filename)
        ...
        print("testing model...")
        #y_predicted = test_lasso(X_normalized, y, lasso_betas, lasso_intercept)
        ...
        y_pred_file = task + "_var_mask02_modular_y_predicted_test_cj_" + time_snippet + ".pckl"
        output_path = os.path.join(output_data_path, session, "models", "outputs")
        save_pickle(output_path, y_pred_file, [y, y_predicted])
            
    #save cv metrics
    output_path = os.path.join(output_data_path, "results", "var", time_snippet)
